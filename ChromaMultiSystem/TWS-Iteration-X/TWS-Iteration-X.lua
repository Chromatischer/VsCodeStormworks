...
function onTick()
	...
	if SelfIsSelected then
		...
		for i = 0, 2 do
			...
			tgtWorldPos = radarToRelativeVec3(
				distance,
			...
			if timeSinceDetected ~= 0 then
				...
				rawRadarData[i + 1] = scalarDivideVec3(
					addVec3(tgtWorldPos, scaleVec3(rawRadarData[i + 1], timeSinceDetected - 1)),
					timeSinceDetected
				) ---@type Vec3
			elseif vec3length(tgtWorldPos) > 50 then
				--Convert the relative position to a global position and add it to the contacts table
				table.insert(contacts, addVec3(tgtWorldPos, Vec3(gpsX, gpsY, gpsZ)))
