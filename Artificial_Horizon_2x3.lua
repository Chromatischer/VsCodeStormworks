...
function onTick()
    ...
    if debug then print("pitch: [" .. pitch .. "] speed: [" .. speed .. "] altitude: [" .. altimeter .. "] roll: [" .. roll .. "]") end
end
...
function onDraw()
    ...
    if speed > 12 then
        ...
        screen.drawLine(screenCenterX + steadyIndicatorWidth, screenCenterY, screenCenterX, screenCenterY + steadyIndicatorHeight)
        screen.drawLine(screenCenterX - steadyIndicatorWidth, screenCenterY, screenCenterX, screenCenterY + steadyIndicatorHeight)
        screen.drawLine(screenCenterX  - steadyIndicatorWidth, screenCenterY, screenCenterX - steadyIndicatorWidth - steadyIndicatorLength, screenCenterY)
        screen.drawLine(screenCenterX  + steadyIndicatorWidth, screenCenterY, screenCenterX + steadyIndicatorWidth + steadyIndicatorLength, screenCenterY)
    else
    ...
    screen.drawText(-3, screenCenterY - 10, string.formatNumberAsInteger(math.roundToDecimal(speed * 3.6 + 10, -1), 3, " "))
    screen.drawText(-3, screenCenterY - 16, string.formatNumberAsInteger(math.roundToDecimal(speed * 3.6 + 20, -1), 3, " "))
    if speed * 3.6 > 20 then
        screen.drawText(-3, screenCenterY + 6, string.formatNumberAsInteger(math.roundToDecimal(speed * 3.6 - 10, -1), 3, " "))
        screen.drawText(-3, screenCenterY + 12, string.formatNumberAsInteger(math.roundToDecimal(speed * 3.6 - 20, -1), 3, " "))
    end
    ...
    screen.setColor(200, 200, 200)
    screen.drawText(Swidth - 21, screenCenterY - 10, string.formatNumberAsInteger(math.roundToDecimal(altimeter + 100, -2), 4, "0"))
    screen.drawText(Swidth - 21, screenCenterY - 16, string.formatNumberAsInteger(math.roundToDecimal(altimeter + 200, -2), 4, "0"))
    if altimeter > 200 then
        screen.drawText(Swidth - 21, screenCenterY + 6, string.formatNumberAsInteger(math.roundToDecimal(altimeter - 100, -2), 4, "0"))
        screen.drawText(Swidth - 21, screenCenterY + 12, string.formatNumberAsInteger(math.roundToDecimal(altimeter - 200, -2), 4, "0"))
    end

    drawThingIndicator(14, screenCenterY, Swidth - 26, math.clamp(altimeterDiff / maxAltimeterDiff, -1, 1), true, enableShadow, math.abs(altimeterDiff) < 0.1 * maxAltimeterDiff)
    --#endregion
...
function drawThingIndicator(speedGainIndicatorLength, speedGainIndicatorCenterY, speedGainIndicatorCenterX, speedDiffFactor, shadowToLeft, enableShadow, isGrayScale)
    ...
    if enableShadow then
        ...
        if not shadowToLeft then
            screen.drawRectF(speedGainIndicatorCenterX, speedGainIndicatorCenterY - speedGainIndicatorLength + 1, 2, 2 * speedGainIndicatorLength)
        else
            screen.drawRectF(speedGainIndicatorCenterX - 1, speedGainIndicatorCenterY - speedGainIndicatorLength + 1, 2, 2 * speedGainIndicatorLength)
        end
    end
    for o = 0, 1, 1 do
    ...
    screen.drawRectF(speedGainIndicatorCenterX - 1, speedGainIndicatorCenterY + speedDiffFactor * speedGainIndicatorLength, 3, 1)
end
