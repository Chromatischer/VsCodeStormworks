...
function onTick()
	...
	tgtWorldPos = radarToRelativeVec3(contactDistance, contactAzimuth, contactElevation, selfAngle, selfPitch)
	if contactTSD ~= 0 then
		rawRadarData = scalarDivideVec3(addVec3(tgtWorldPos, scaleVec3(rawRadarData, contactTSD - 1)), contactTSD) ---@type Vec3
	else
...
function onDraw()
	...
	screen.drawLine(
		Swidth / 2,
		Sheight / 2,
		Swidth / 2 + 50 * math.cos(math.rad(radarRotation)),
		Sheight / 2 + 50 * math.sin(math.rad(radarRotation))
	)
end
